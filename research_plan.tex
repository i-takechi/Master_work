...
Reflection Detection in RGB and Lidar Data}）\cite{zhao2024_3dref}を使用する。
データセットの概要と本研究での用途を表\ref{tab:dataset-3dref}に示す。

\begin{table}[h]
  \centering
  \caption{使用するデータセット3DRefの概要と用途}
  \label{tab:dataset-3dref}
  \begin{tabular}{p{0.20\linewidth}p{0.40\linewidth}p{0.30\linewidth}}
    \toprule
    項目 & 3DRefの内容 & 本研究での用途 \\
    \midrule
    収録環境
      & 鏡、ガラス、白板、モニタなどの反射面を含む屋内環境
      & 鏡面・ガラス反射によるghost観測の分析 \\
    データ規模
      & 3系列の屋内シーケンス、48,024個のラベル付きLiDAR点群、3,799枚のラベル付きRGB画像
      & sequence単位の学習・検証・評価 \\
    LiDARセンサ
      & Ouster、Livox、Hesaiの3種類のmulti-return LiDAR
      & センサの違いを含むLiDAR-onlyモデルの入力 \\
    RGB画像
      & LiDAR観測に対応する屋内環境のカラー画像
      & LiDAR--Camera融合モデルの入力 \\
    Semantic label
      & 点単位の3Dラベルおよび画像上の2Dラベル
      & ghost確率推定モデルの教師信号 \\
    センサ姿勢・校正
      & LiDAR poseおよびカメラの内部・外部パラメータ
      & 世界座標への変換およびLiDAR点と画像特徴の対応付け \\
    3D ground truth
      & Semantic label付き3D mesh
      & 物理占有状態のground truth構築 \\
    出典
      & 3DRef公式論文・プロジェクトページ\cite{zhao2024_3dref}
      & データ仕様および公式ベンチマークの参照 \\
    \bottomrule
  \end{tabular}
\end{table}

3DRefの公開ベンチマークでは、LiDAR点群セグメンテーションとしてMinkowskiNet、
SPVCNNおよびCylinder3Dが比較され、return情報を追加したCylinder3Dが全センサ統合条件で
最も高いmIoUを示している\cite{zhao2024_3dref}。したがって本研究では、
Cylinder3Dを再現可能な公式ベースラインとして使用し、後述するTransformer型モデルと
比較する。

\section{関連研究と採用方針}

近年のLiDAR--Camera融合では、両モダリティを共通のBird's-Eye View（BEV）へ変換して
統合する方法が3D物体検出およびBEV map segmentationの主要な構成となっている
\cite{liu2023_bevfusion}。しかし、本研究は屋内の壁面、鏡、ガラスおよびその前後に
分布する点を点単位で分類するため、高さ方向を圧縮するBEV表現を主表現とはしない。
BEV型手法の「共通座標で統合する」という原理を採用しつつ、3D point/voxel tokenを
保持する。

LiDAR特徴抽出には、屋内外の点群理解で高い性能と計算効率を示すPTv3
\cite{wu2024_ptv3}を採用する。カメラ特徴抽出には、dense predictionに適した
自己教師あり特徴を持つDINOv3\cite{simeoni2025_dinov3}を使用し、3DRefで再学習可能な
反射面segmentationモデルEBLNet\cite{he2021_eblnet}の出力を補助特徴として加える。

融合では、calibrationに基づく単一点対応だけに依存せず、投影位置周辺から必要な画像特徴を
選択するcross-attentionを用いる。これは、LiDAR queryによるsoft associationを用いて
画像劣化や位置ずれへの頑健性を高めるTransFusion\cite{bai2022_transfusion}の考え方を、
点単位reflection segmentationへ適用するものである。

時系列処理については、過去のLiDAR点と画像特徴をpose補償して利用するTASeg
\cite{wu2024_taseg}、および共有3D位置符号化とcross-modal memory attentionを用いる
SAM4D\cite{xu2025_sam4d}を参照する。ただし、3DRefは3系列であり大規模な
end-to-end時系列Transformerは過学習しやすいため、本研究では短い時間窓に限定した
Sparse Temporal Transformerとする。

3DRef上ではLiDAR--Camera融合による点単位reflection segmentationの標準的な
leaderboardがまだ確立していない\cite{zhao2024_3dref}。したがって、異なるタスクの
最高値を直接「本研究の最高性能」とみなさず、Cylinder3D、PTv3、単純特徴連結、
cross-attention、時系列統合を同一分割で段階的に比較する。
...
\begin{description}
  ...
  \item[RQ3]
  世界座標で整列した短時間のLiDAR点特徴および画像特徴をSparse Temporal
  Transformerで統合することで、単一フレームモデルと比較してghost点の識別性能と
  確率の時間的一貫性は向上するか。
...
取得する。ただし、鏡面反射によるghost点は鏡の奥側の仮想位置に生成されるため、
通常の実在点と同じ意味での点--画素対応が成立するとは限らない。また、センサ間の視差、
時刻ずれ、calibration誤差および遮蔽により、単一画素からのhard samplingは不安定に
なり得る。このため、単純連結を比較用ベースラインとし、主手法では投影位置周辺の
複数画像tokenから特徴を選択する局所cross-attentionを用いる。
...
主手法では、投影位置$\bm{u}_{i,t}$の周辺領域
$\mathcal{N}(\bm{u}_{i,t})$から得る画像tokenを
$\bm{K}_{i,t}^{\mathrm{C}}$および$\bm{V}_{i,t}^{\mathrm{C}}$とし、
LiDAR点特徴をqueryとする。
\begin{align}
  \bm{c}_{i,t}
  =
  ...
  \left(
    \bm{W}_{q}\bm{f}_{i,t}^{\mathrm{L}},
    \bm{K}_{i,t}^{\mathrm{C}},
    \bm{V}_{i,t}^{\mathrm{C}}
  \right).
\end{align}
このsoft associationは、単一の投影対応へ強く依存せずに有用な画像特徴を選択する
構成であり、Transformerを用いたLiDAR--Camera融合で有効性が示されている
\cite{bai2022_transfusion}。
...
\begin{enumerate}
  \item 3DRefから、LiDAR点群、RGB画像、点単位ラベル、センサ姿勢および
        calibration情報を取得する。
  \item PTv3を用いて、XYZIR点群からLiDAR点特徴を抽出する。
  \item 事前学習済み画像encoderを用いて、RGB画像からdenseな画像特徴を抽出する。
  \item LiDAR-only分岐ではLiDAR点特徴から、LiDAR--Camera分岐では局所
        cross-attentionおよび信頼度gateによる融合特徴から、6クラスlogitsを出力する。
  \item LiDAR poseで整列した近傍フレームのvoxel tokenをSparse Temporal
        Transformerで統合し、点単位logitsを更新する。
  \item 各分類器の最終logitsを、validationデータを用いて個別に確率校正する。
  \item 各点について、
...
6クラスのlogitsを出力する。主手法のbackboneにはPoint Transformer V3
（PTv3）\cite{wu2024_ptv3}を用いる。PTv3は点群を空間充填曲線によりserializationし、
広い受容野を持つattentionを効率的に計算できるため、鏡・ガラス面の局所的なreturn特性と、
面の前後に分布するghost点の広域的な幾何関係を同時に扱えると期待される。
入力特徴にはXYZIRに加え、sensor ID、rangeおよび有効return maskを用いる。

一方、3DRef公式ベンチマークで高い性能を示すCylinder3Dを非Transformer型の
主要ベースラインとする\cite{zhao2024_3dref}。これにより、Transformerの導入効果と
LiDAR--Camera融合の効果を分離して評価する。
\begin{align}
...
\begin{align}
  ...
  \right)
\end{align}
とする。ここで、単純連結は比較用ベースラインとし、主手法の$\Phi(\cdot)$には
局所cross-attentionと信頼度gateを使用する。

\subsection{画像encoder}

画像encoderには、自己教師あり学習によりdenseな局所特徴を獲得したDINOv3
\cite{simeoni2025_dinov3}を用いる。3DRefで利用可能なRGB画像数はLiDARフレーム数より
少ないため、初期実装では事前学習済みencoderを凍結し、特徴射影層および融合層のみを
学習する。その後、validation性能を監視しながら上位blockのfine-tuningを検討する。

また、画像上のglass、mirrorおよびother reflectiveの位置を明示的に与える補助特徴として、
3DRefで再学習したEBLNetによる反射面確率mapを使用する
\cite{he2021_eblnet,zhao2024_3dref}。DINOv3特徴のみ、反射面確率のみ、両者の併用を
ablationで比較する。

\subsection{局所cross-attentionと信頼度gate}

点$i$の投影位置周辺にある複数スケールの画像tokenに対して、LiDAR点特徴をqueryとする
局所cross-attentionを適用する。融合された画像文脈$\bm{c}_{i,t}$の利用率を、
\begin{align}
  \rho_{i,t}
  =
  \sigma
  ...
  \right)
\end{align}
とする。ここで、$H_{i,t}^{\mathrm{C}}$は画像分岐の予測entropyまたは不確実性である。
最終的な融合特徴は、
\begin{align}
  \bm{f}_{i,t}^{\mathrm{LC}}
  =
  ...
  \bm{W}_{o}\bm{c}_{i,t}
\end{align}
とする。画像外、遮蔽、画像側の高entropyまたは投影不整合がある場合には
$\rho_{i,t}$を小さくし、LiDAR主体の推定へ連続的に戻れる構成とする。
...
\section{第2段階：短期時系列Transformer}

\subsection{世界座標で整列した時系列token}

単一フレームの点特徴だけでなく、LiDAR poseを用いて近傍フレームを現在時刻の
世界座標へ整列し、voxelごとに特徴を集約する。時刻$t$のvoxel $v$におけるtokenを
$\bm{h}_{v,t}^{m}$とし、過去$K$フレームの同一または近傍voxelを参照する
Sparse Temporal Transformerを構築する。
\begin{align}
  \hat{\bm{h}}_{v,t}^{m}
  =
  ...
  \left(
    ...
    \left\{
      ...
      \left(
        ...
        \Delta t
      \right)
    \right\}_{(v',k)\in\mathcal{N}_{v,t}}
  \right).
\end{align}
ここで、$\mathcal{N}_{v,t}$は空間近傍と時間窓で制限した参照token集合である。
全点間attentionは計算量が大きく、3DRefの系列数に対して過学習しやすいため使用しない。
初期実装では$K=3$または$5$とし、静的環境を主対象としてpose補償後の局所attentionを
行う。

時間的LiDAR点と画像特徴の統合はLiDAR semantic segmentationにおいて高い性能を示して
おり\cite{wu2024_taseg}、共有3D位置表現とmemory attentionを組み合わせる設計も
提案されている\cite{xu2025_sam4d}。本研究ではこれらの考え方を小規模な
voxel-level attentionとして実装し、点特徴へ補間して最終logitsを出力する。

\subsection{短期学習と長期確率統合の役割分担}

Sparse Temporal Transformerは数フレームの文脈から点単位分類を改善する
学習型の短期統合として用いる。一方、後述するBeta evidence fusionは、校正済み確率を
長期間にわたって解釈可能な形で蓄積する地図側の統合として維持する。すなわち、
\begin{align}
  \text{single-frame feature}
  \rightarrow
  \text{temporal feature}
  \rightarrow
  \text{calibrated probability}
  \rightarrow
  \text{Beta evidence fusion}
\end{align}
の順に処理する。

\section{第3段階：ghost確率の校正}

各分類器のsoftmax出力は、実際の正解頻度と一致するとは限らない。そのため、
LiDAR-only分類器とLiDAR--Camera融合分類器について、短期時系列処理後の最終logitsに
個別にTemperature Scalingを行う。単一フレーム条件については単一フレームlogitsを
校正し、異なる推論条件のtemperatureを共有しない。
...
\section{第5段階：voxel単位のghost確率統合}
...
各モデルから、対応する
$q_{i,t}^{m}$、$\gamma_{v,t}^{m}$および$o_{v,t}^{m}$を構築する。
主比較では、LiDAR-onlyとLiDAR--CameraでPTv3の容量、時間窓、学習データおよび
地図更新条件を共通化する。
...
\begin{itemize}
  ...
  \item XYZI／XYZIR
  \item Cylinder3D／PTv3
  \item 二値分類／6クラス分類
  \item 画像特徴なし／DINOv3特徴／EBLNet反射面確率／両者
  \item DINOv3凍結／上位blockのfine-tuning
  \item 単純な特徴連結／局所cross-attention
  \item 信頼度gateなし／あり
  \item 単一スケール画像token／複数スケール画像token
  \item 未校正softmax／校正済み確率
  \item 単一フレーム／Sparse Temporal Transformer
  \item 時間窓$K=1,3,5$
...
Cylinder3D＋XYZIRの公式ベースラインを再現した後、PTv3＋XYZIRのLiDAR-only分類器を
学習し、
\begin{align}
  q_{i,t}^{\mathrm{L,raw}}
\end{align}
...
完了条件は、両backboneについて任意フレームの6クラスlogitsを保存し、
ReflectionクラスのPrecision、Recall、IoUおよびAUPRCを同一testデータで比較できる
ことである。

\subsection{Phase 3：TransformerによるLiDAR--Camera融合}

DINOv3画像encoder、EBLNet反射面確率map、LiDAR点と局所画像tokenの対応付け、
cross-attentionおよび信頼度gateを実装し、
\begin{align}
  q_{i,t}^{\mathrm{LC,raw}}
\end{align}
を出力する。

完了条件は、単純特徴連結と提案融合手法をLiDAR-only手法と同一のtestデータ上で比較し、
画像特徴が無効または不確実な点におけるgateの挙動を可視化できることである。

\subsection{Phase 4：短期時系列Transformer}

LiDAR poseによって近傍フレームを整列し、voxel tokenに対するSparse Temporal
Transformerを実装する。単一フレーム、$K=3$および$K=5$を比較する。

完了条件は、各モダリティについて単一フレームと時系列モデルのReflection IoU、
AUPRCおよび計算量を比較できることである。

\subsection{Phase 5：確率校正}

単一フレームおよび時系列モデルの最終logitsに対して、それぞれTemperature Scalingを行い、
\begin{align}
  q_{i,t}^{\mathrm{L}},
  \qquad
  q_{i,t}^{\mathrm{LC}}
...
\subsection{Phase 6：世界座標統合とghost確率地図}
...
\subsection{Phase 7：物理占有確率と最終評価}
...
\begin{enumerate}
  ...
  \item PTv3による点群特徴、DINOv3および反射面確率による画像特徴を、局所
        cross-attentionと信頼度gateで統合し、反射環境特有の不確かな点--画素対応を
        softに扱うこと。
  \item LiDAR-onlyとLiDAR--Camera融合について、点単位の分類性能だけでなく、
        確率校正性能を比較すること。
  \item Sparse Temporal Transformerによる短期特徴統合と、Beta evidence fusionによる
        長期確率統合を役割分担させ、ghost確率を確率的デジタルツインの状態として
        保持すること。
